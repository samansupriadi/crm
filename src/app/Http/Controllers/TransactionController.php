...
class TransactionController extends Controller
{
    ...
    public function show(Transaction $transaction){
        return new TransactionResource($transaction->loadMissing(
            'detailTransactions.program', 'detailTransactions.savingDetails'
        ));
    }
    ...
    public function approval(Request $request, Transaction $transaction){
        $validated = $request->validate([
            'approval' => 'required|' . Rule::in(['Approved', 'unApprove']),
        ]);
        ...
        try {
            $transaction->update([
                ...
                'approved_by'       => $request->approval == 'Approved' ?  Auth::user()->id : NULL,
                'reject_by'         => $request->approval == 'unApprove' ? Auth::user()->id : NULL,
            ]);
            DB::commit();
            return $this->success([
    ...
    //end point untuk keperluan api update linked to
