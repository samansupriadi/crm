...
usage_docs() {
  echo "Usage: $0 INPUT_OWNER=<owner> INPUT_REPO=<repo> INPUT_GITHUB_TOKEN=<github_token> INPUT_WORKFLOW_FILE_NAME=<workflow_file_name>"
}
...
for arg in "$@"; do
  if [[ "$arg" =~ owner=([^[:space:]]+) ]]; then
    INPUT_OWNER="${BASH_REMATCH[1]}"
  elif [[ "$arg" =~ repo=([^[:space:]]+) ]]; then
    INPUT_REPO="${BASH_REMATCH[1]}"
  elif [[ "$arg" =~ github_token=([^[:space:]]+) ]]; then
    INPUT_GITHUB_TOKEN="${BASH_REMATCH[1]}"
  elif [[ "$arg" =~ workflow_file_name=([^[:space:]]+) ]]; then
    INPUT_WORKFLOW_FILE_NAME="${BASH_REMATCH[1]}"
  fi
done
